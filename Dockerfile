...
COPY --from=builder /app/target/fs-contractTesting-svc-*.jar app.jar
...
HEALTHCHECK --interval=30s --timeout=3s --start-period=40s --retries=3 \
  CMD wget --no-verbose --tries=1 --spider http://localhost:${PORT}/contractTesting/actuator/health || exit 1
